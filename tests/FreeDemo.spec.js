...
test.describe('FreeDemo', () => {
  test('Free Demo approved flow', async ({ browser }) => {
    test.setTimeout(300000);

    const baseUrl              = process.env.BASE_URL               || 'https://app.mondofi.co';
    const userName             = process.env.USER_NAME;
    const password             = process.env.PASSWORD;
    const demoEmail            = process.env.DEMO_EMAIL             || 'FinnLand@mailinator.com';
    const demoShortname        = process.env.DEMO_SHORTNAME         || 'FL123';
    const demoFirstName        = process.env.DEMO_FIRST_NAME        || 'Financial';
    const demoLastName         = process.env.DEMO_LAST_NAME         || 'World';
    const building_name        = process.env.BUILDING_NAME          || 'Playwright world';
    ...
    console.log(`\n${HR_THICK}`);
    console.log(`  ${C.bold}FreeDemo  ·  Approved Flow${C.reset}`);
    console.log(HR_THIN);
    ...
    // ── Fill & Submit Demo Form ────────────────────────────────────────────────
    await step('Fill & Submit Demo Form', async () => {
      for (let i = 0; i <= 15; i++) {
      ...
      await page.fill('xpath=//input[@name="buildingName"]', building_name);
    ...
    await step('Super Admin Approval', async () => {
    ...
    // ── Mailcatcher Verification ────────────────────────────────────────────────
    await step('Mailcatcher Verification', async () => {
      ...
      if (!emailFrame) throw new Error('Could not find email body iframe in Mailcatcher');

      const [newPage] = await Promise.all([
        context.waitForEvent('page', { timeout: 30000 }),
        emailFrame.click('xpath=//td[@class="button-td button-td-primary"]'),
      ]);
      confirmPage = newPage;
    }, { critical: true });

    // ── Confirm Account Login ───────────────────────────────────────────────────
    await step('Confirm Account Login', async () => {
